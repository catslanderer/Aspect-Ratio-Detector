...
2. App AR is always ~3/4 or ~4/3 (these two ARs cover most use cases while requiring the user to be familiar with at most two app layouts)
...
- Ask the user to resize their browser window or reorient their device to better fit the app. This accomplishes goals 1-5 while obviously failing to accomplish goal 6. The downsides are that it maybe be annoying to or impossible for a user to adjust the viewport. Furthermore, reshaping the viewport to fit the app clearly doesn't maximize the use of available screen area anyway (see '3/4 app on a 16/9 monitor' in the image below for an example).
...
For this project I'm not interested in exact ARs, but in setting up ranges. Specifically, I want the design to respond to viewport ARs in the following four ranges:
...
If you've noticed that multiple rules apply when the AR is _exactly_ 3/4, 1/1, or 4/3, then good for you for paying attention! If you caught that, then you may already know that CSS rules are applied in order, and that the last rule (in the last stylesheet, if the rule appears in multiple stylesheets) will prevail. In this case an exact 3/4 AR would have the second set of rules applied, an exact 1/1 would have the third set of rules applied, and so on. Note that this means you should be careful if you use overlapping aspect ratio media queries in multiple places in your CSS. In most cases, I imagine, you'd want to make sure that your ranges of ARs are in the same order each time you use them.

So now we can apply CSS rules selectively based on the detected AR of the viewport. This alone is a useful feature to understand, but it isn't enough to meet all six goals outlined earlier - we still need the app to respond to viewport size.

## Responsiveness to either viewport width _or_ height

Since we're aiming for the board and controls components to be as large as possible given the viewport size and aspect ratio—while maintaining a square shape for the Board component—we're going to have get to our hands dirty with careful calculations and responsive units for each AR range. We'll start with the `< 3/4` range.

### Viewport aspect ratio: < 3/4 range

In this project the App component is the container for the other components, and we can base its width and height first on the AR of the viewport. For the `< 3/4` range, we will use the version of the app whose AR is ~3/4. The following code will accomplish this (`display: grid` and the `grid-area` assignments are not shown):
...
> So far so good: at a 5/8 ratio there is unused space, but given that the board component must be square, the app can't be any larger than it is. Since one of the app's ARs is ~3/4, it naturally fills most of the viewport when the viewport's AR is ~3/4.
...
I've accomplished the six goals I laid out earlier, and hopefully explained clearly how I did so. There is more to know about aspect ratio media queries than I discuss here, but if you're new to them, this article will perhaps serve as a useful introduction. Though the use case I've explored is a bit niche, I think it's simple enough to be adapted to other cases. If you've learned something, spotted an error, or if I've saved you a bit of effort in solving a similar puzzle, let me know in the comments.
